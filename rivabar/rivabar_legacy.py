"""
DEPRECATED: This is the original monolithic rivabar.py file (5,000+ lines).
...
Please use the new modular imports instead:
    ...
    from rivabar import extract_centerline
    # or
    from rivabar.core import extract_centerline
...
from shapely.geometry import Point, Polygon, LineString, LinearRing, MultiPolygon, MultiLineString, GeometryCollection
...
def plot_im_and_lines(im, left_utm_x, right_utm_x, lower_utm_y, upper_utm_y, G_rook, 
    ...
    for i in trange(2, len(G_rook.nodes)):
        ...
        if smoothing:
            ...
            x, y = remove_endpoints(interior_lstr, remove_count=1)
            interior_lstr = LineString(zip(x, y)).simplify(3)
            x = interior_lstr.xy[0]
            y = interior_lstr.xy[1]
        plt.plot(x_interior, y_interior, '.-', color='tab:blue')
    if plot_lines:
...
def smooth_line(x, y, savgol_window=21): #, spline_ds = 25, spline_smoothing = 10000, savgol_window = 21, savgol_poly_order = 3):
    """
    ...
    tolerance = vertex_density_tolerance(smooth_line, multiplier=0.5)
...
def smooth_banklines(G_rook, dataset, mndwi, save_smooth_lines=False):
    ...
    for i in range(2): # deal with the main banklines first - they are more complicated
        ...
        x1, y1 = smooth_line(line.xy[0], line.xy[1])
